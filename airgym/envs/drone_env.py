...
class AirSimDroneEnv(AirSimEnv):
    def __init__(self, ip_address, step_length, image_shape, ident):
    ...
    def _get_obs(self):
        responses = self.drone.simGetImages([self.image_request],vehicle_name=self.ident)
        image = self.transform_obs(responses)
        self.drone_state = self.drone.getMultirotorState(vehicle_name=self.ident)
    ...
    def _do_action(self, action):
        ...
        quad_vel = self.drone.getMultirotorState(vehicle_name=self.ident).kinematics_estimated.linear_velocity
        self.drone.moveByVelocityAsync(
            quad_vel.x_val + quad_offset[0],
            quad_vel.y_val + quad_offset[1],
            quad_vel.z_val + quad_offset[2],
            5,vehicle_name=self.ident,
        ).join()
